...
<body>
   ...
   <script>
    document.addEventListener('DOMContentLoaded', function() {
        // تعريف وظائف البحث والتكبير/التصغير هنا بعد تحميل DOM
        ...
        document.querySelector('.search-box').addEventListener('input', function() {
            ...
            const suraItems = document.querySelectorAll('.sura-list li');
            
            suraItems.forEach(function(item) {
...
document.querySelectorAll('.zoom-btn').forEach(button => {
    button.addEventListener('click', function() {
        const zoomIn = this.title === 'تكبير';
        const verseElements = document.querySelectorAll('.ayah');  // تحديد الآيات للتغيير فيها
        verseElements.forEach(verse => {
            let currentFontSize = parseFloat(window.getComputedStyle(verse).fontSize);  // الحصول على حجم الخط الحالي
            if (zoomIn) {
                verse.style.fontSize = (currentFontSize * 1.2) + 'px';  // تكبير حجم الخط بنسبة 20%
            } else {
                verse.style.fontSize = (currentFontSize / 1.2) + 'px';  // تصغير حجم الخط بنسبة 20%
            }
        });
    });
});
